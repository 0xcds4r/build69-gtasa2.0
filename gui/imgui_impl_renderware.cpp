...
{
	const RwReal nearScreenZ = 		*(RwReal*)(g_GTASAAdr+0xA7C348);	// CSprite2d::NearScreenZ 009DAA60
	const RwReal recipNearClip = 	*(RwReal*)(g_GTASAAdr+0xA7C344);	// CSprite2d::RecipNearClip 009DAA64
	...
	if(!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
	{
		...
		if(!g_pVB)
		{
			FLog("GUI | Error: couldn't allocate vertex buffer (size: %d)", g_VertexBufferSize);
			return;
		}
		FLog("GUI | Vertex buffer reallocated. Size: %d", g_VertexBufferSize);
	}
...
bool ImGui_ImplRenderWare_Init()
{
	...
	FLog("GUI | Display size: %f, %f", io.DisplaySize.x, io.DisplaySize.y);
...
{
	FLog("GUI | CreateDeviceObjects.");
	if(g_FontRaster) FLog("GUI | Warning: Font raster != 0");
	...
	FLog("GUI | Font atlas width: %d, height: %d, depth: %d", width, height, bytes_per_pixel*8);
